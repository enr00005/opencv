import cv2
import numpy as np

# Load YOLO
net = cv2.dnn.readNet("yolov3.weights", "yolov3.cfg")
layer_names = net.getLayerNames()
output_layers = [layer_names[i - 1] for i in net.getUnconnectedOutLayers()]


    # #*download yoyo dirctly using python*

# import urllib.request

# url = "https://data.pjreddie.com/files/yolov3.weights"
# print("Downloading YOLOv3 weights (~236 MB)...")
# urllib.request.urlretrieve(url, "yolov3.weights")
# print("Download complete!")


# Load COCO dataset class labels
with open("coco.names", "r") as f:
    classes = [line.strip() for line in f.readlines()]

# Load video / webcam
cap = cv2.VideoCapture(0)  # use 0 for webcam, or give video path

while True:
    ret, frame = cap.read()
    height, width, channels = frame.shape

    # Prepare input blob for YOLO
    blob = cv2.dnn.blobFromImage(frame, 0.00392, (416, 416), (0, 0, 0), True, crop=False)
    net.setInput(blob)
    outs = net.forward(output_layers)

    class_ids, confidences, boxes = [], [], []

    # Process detections
    for out in outs:
        for detection in out:
            scores = detection[5:]
            class_id = np.argmax(scores)
            confidence = scores[class_id]
            # Filter only animals (dog, cat, horse, cow, sheep, elephant, bear, zebra, giraffe, etc.)
            animal_classes = ["dog", "cat", "bird", "horse", "sheep", "cow", "elephant", "bear", "zebra", "giraffe"]
            if confidence > 0.5 and classes[class_id] in animal_classes:
                center_x = int(detection[0] * width)
                center_y = int(detection[1] * height)
                w = int(detection[2] * width)
                h = int(detection[3] * height)
                x = int(center_x - w / 2)
                y = int(center_y - h / 2)

                boxes.append([x, y, w, h])
                confidences.append(float(confidence))
                class_ids.append(class_id)

    # Non-maximum suppression to reduce duplicate boxes
    indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.5, 0.4)

    font = cv2.FONT_HERSHEY_SIMPLEX
    for i in range(len(boxes)):
        if i in indexes:
            x, y, w, h = boxes[i]
            label = str(classes[class_ids[i]])
            confidence = round(confidences[i] * 100, 2)
            color = (0, 255, 0)
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
            cv2.putText(frame, f"{label} {confidence}%", (x, y - 10), font, 0.6, color, 2)

    cv2.imshow("Animal Detection", frame)

    # Press 'q' to quit
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()
